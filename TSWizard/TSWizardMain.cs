        ...
        private void Software_update_checker(int __software_mode, string __software_version, bool __installed_status, bool __notify_mode){
            try{
                ...
                using (WebClient getLastVersion = new WebClient()){
                    long client_num_version = TS_VersionParser.ParseNormalizeCheck(__software_version);
                    string last_version_get = getLastVersion.DownloadString(ts_software_version_links[__software_mode]).Split('=')[1].Trim();
                    long last_num_version = TS_VersionParser.ParseNormalizeCheck(last_version_get);
                    //
        ...
        private void Software_update_check(int _check_update_ui){
            try{
                ...
                using (WebClient getLastVersion = new WebClient()){
                    ...
                    long client_num_version = TS_VersionParser.ParseNormalizeCheck(Application.ProductVersion);
                    string last_version_get = getLastVersion.DownloadString(TS_LinkSystem.github_link_lv).Split('=')[1].Trim();
                    string last_version_raw = TS_VersionParser.ParseUINormalize(last_version_get);
                    long last_num_version = TS_VersionParser.ParseNormalizeCheck(last_version_get);
                    if (client_num_version < last_num_version){
                        ...
                        string update_msg = client_num_version == last_num_version ? string.Format(software_lang.TSReadLangs("SoftwareUpdate", "su_not_available"), Application.ProductName, "\n", client_version_raw) : string.Format(software_lang.TSReadLangs("SoftwareUpdate", "su_newer"), "\n\n", $"v{client_version_raw}");
