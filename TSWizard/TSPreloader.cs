    ...
    public partial class TSPreloader : Form{
        ...
        */
            ...
            try{
                ...
                if (!File.Exists(ts_sf)){
                    try{
                        ...
                        // WINDOW BEHAVIOR
                        software_settings_save.TSWriteSettings(ts_settings_container, "WindowBehavior", "0");
                        // SET UPDATE NOTIFICATIONS
                        software_settings_save.TSWriteSettings(ts_settings_container, "UpdateNotifications", "1");
                        // SET ARCHITECTURE MODE
                        software_settings_save.TSWriteSettings(ts_settings_container, "ArchitectureMode", "2");
                    }catch (Exception ex){
                        // SET ERROR LOG
                        LogError(ex);
                    }
        ...
        private async Task load_animation(){
            ...
            await Task.Run(() => {
                Invoke(new Action(() => {
                    TS_Wizard ts_wizard = new TS_Wizard();
                    ts_wizard.Show();
                    //
                    Hide();
                }));
